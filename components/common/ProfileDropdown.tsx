...
const ProfileDropdown: React.FC<ProfileDropdownProps> = ({ profile, onSignOut, onSelectRole, onProfileClick }) => {
    ...
    const discoverIdentities = useCallback(async () => {
        ...
        try {
            ...
            if (!error && data) {
                const found = new Set<string>(data.filter((r: any) => r.is_completed).map((r: any) => r.role_name));
                // Current role is definitely completed if we are here
    ...
    const restrictedForCreation: string[] = [
        BuiltInRoles.SUPER_ADMIN
    ];
    
    // Roles you can switch TO (ones you have already completed, excluding current)
    ...
    // Roles you can START (ones you haven't completed yet)
    const creatableIdentities = useMemo(() => roles
        .filter(r => !completedRoles.has(r) && !restrictedForCreation.includes(r)), 
    [roles, completedRoles, restrictedForCreation]);
        ...
        <div className="relative z-50" ref={dropdownRef}>
            <button onClick={() => setIsOpen(!isOpen)} className={`flex items-center gap-3 pl-1 pr-3 py-1 rounded-full transition-all group ${isOpen ? 'bg-muted shadow-inner' : 'hover:bg-muted/50'}`}>
                ...
                <div className="hidden md:flex flex-col items-start text-left mr-1">
                    ...
                    <span className="text-[10px] font-medium text-muted-foreground max-w-[100px] truncate uppercase tracking-wider">{profile.role || 'Setup Required'}</span>
                </div>
                    ...
                    <div className="flex flex-col">
                        <div className="p-6 flex items-center gap-4 border-b border-white/5">
                             ...
                             <div className="min-w-0">
                                 ...
                                 <div className="mt-2.5">
                                     <span className="px-3 py-1 rounded-lg bg-primary/10 text-primary text-[9px] font-black uppercase tracking-widest border border-primary/20">{profile.role || 'New User'}</span>
                                 </div>
                             </div>
                        </div>
                        <div className="p-2 border-b border-white/5">
                             <button onClick={() => { onProfileClick?.(); setIsOpen(false); }} className="w-full flex items-center gap-3 p-3 rounded-xl text-xs font-black text-white/70 hover:text-white hover:bg-white/5 transition-all uppercase tracking-widest group">
                                 <SettingsIcon className="w-4 h-4 text-white/20 group-hover:text-primary transition-colors" /> Manage Credentials
                             </button>
                        </div>
                        <div className="flex-grow max-h-[420px] overflow-y-auto custom-scrollbar">
                            {checkingRoles ? (
                                <div className="py-16 flex flex-col items-center justify-center gap-4"><Spinner size="md" className="text-primary" /><p className="text-[10px] font-black uppercase text-white/20 tracking-[0.3em]">Discovering Identities</p></div>
                            ) : (
                                <>
                                    {switchableIdentities.length > 0 && onSelectRole && (
                                        <div className="p-3">
                                            <p className="px-4 py-3 text-[10px] font-black text-white/20 uppercase tracking-[0.3em]">Switch Active Identity</p>
                                            {switchableIdentities.map(roleName => {
                                                ...
                                                return (
                                                    <button key={roleName} onClick={() => !isCurrent && handleAction(roleName, true)} className={`w-full flex items-center p-3 rounded-xl text-sm transition-all mb-1 ${isCurrent ? 'bg-primary/5 cursor-default' : 'hover:bg-white/5'}`}>
                                                        <div className={`p-2.5 rounded-xl mr-4 ${isCurrent ? 'bg-primary text-white shadow-primary/20' : 'bg-white/5 text-white/20'}`}><Icon className="w-5 h-5"/></div>
                                                        <div className="flex-grow text-left">
                                                            ...
                                                            {isCurrent && <span className="block text-[9px] text-emerald-500 font-black uppercase tracking-[0.2em] mt-0.5">Active</span>}
                                                        </div>
                                    ...
                                    {creatableIdentities.length > 0 && onSelectRole && (
                                        <div className="p-3 mt-1 pt-4 border-t border-white/5">
                                             <p className="px-4 py-3 text-[10px] font-black text-white/20 uppercase tracking-[0.3em]">Initialize New Profile</p>
                                             {creatableIdentities.map(roleName => {
                                                 ...
                                                 return (
                                                     <button key={roleName} onClick={() => handleAction(roleName, false)} className="w-full flex items-center p-3 rounded-xl hover:bg-white/5 transition-all group/item mb-1">
                                                         <div className={`p-2.5 rounded-xl transition-colors bg-white/5 text-white/20 mr-4 group-hover/item:text-indigo-400`}><Icon className="w-5 h-5" /></div>
                                                         <span className="text-xs font-black text-white/60 uppercase tracking-wider group-hover/item:text-white">
                                                            Create {roleName} Profile
                                                         </span>
                                                         {processingRole === roleName ? <Spinner size="sm" className="ml-auto" /> : <PlusIcon className="w-4 h-4 ml-auto text-white/10 group-hover/item:text-indigo-400" />}
                                                     </button>
                                                 )
                                             })}
                                        </div>
                        ...
                        <div className="p-4 bg-black/40 border-t border-white/5">
                             <button onClick={onSignOut} className="w-full flex items-center justify-center gap-3 p-4 rounded-2xl text-[11px] font-black text-red-500 hover:bg-red-500 hover:text-white transition-all uppercase tracking-[0.25em] shadow-lg group">
                                 <LogoutIcon className="w-5 h-5 transform group-hover:-translate-x-1 transition-transform" /> Terminate Session
                             </button>
                        </div>
                    </div>
                </div>
