...
const TeachersManagementTab: React.FC<TeachersManagementTabProps> = ({ profile, branchId }) => {
    ...
    const fetchTeachers = useCallback(async () => {
        ...
        // Map RPC result to TeacherExtended and add mock daily status
    ...
    const StatusBadge = ({ status }: { status?: string }) => {
        ...
        return (
            <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide border ${styles}`}>
                {icon} {status || 'Unknown'}
            </span>
        );
    };
            ...
            <div className="flex flex-col md:flex-row justify-between items-end gap-4">
                ...
                <div className="flex gap-2">
                     <button 
                        ...
                        className="flex items-center gap-2 px-4 py-2.5 bg-background text-foreground font-bold text-sm rounded-xl hover:bg-muted transition-colors border border-border"
                    >
                        <WorkflowIcon className="w-4 h-4 text-muted-foreground" /> Process Guide
                    </button>
                    <button 
                        onClick={() => setIsAiAuditOpen(true)}
                        className="flex items-center gap-2 px-4 py-2.5 bg-violet-100 text-violet-700 dark:bg-violet-900/20 dark:text-violet-300 font-bold text-sm rounded-xl hover:bg-violet-200 dark:hover:bg-violet-900/40 transition-colors border border-violet-200 dark:border-violet-800"
                    >
            ...
            {activeTab === 'departments' ? (
                ...
                <>
                    ...
                    <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden flex flex-col min-h-[500px]">
                        {selectedIds.size > 0 && (
                            <div className="px-6 py-3 bg-primary/5 border-b border-primary/10 flex items-center justify-between animate-in fade-in slide-in-from-top-1 sticky top-0 z-20">
                                            ...
                                            >
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        <div className="relative">
                                                            ...
                                                            <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-background ${
                                                                (teacher as any).dailyStatus === 'Present' ? 'bg-green-500' : 
                                                                (teacher as any).dailyStatus === 'Absent' ? 'bg-red-500' : 'bg-amber-500'
                                                            }`}></div>
                                                        </div>
                                                        <div>
                                                ...
                                                <td className="p-4">
                                                    <div className="flex flex-col gap-1">
                                                        <div className="flex items-center gap-1.5 text-xs text-muted-foreground"><MailIcon className="w-3 h-3"/> <span className="truncate max-w-[150px]" title={teacher.email}>{teacher.email}</span></div>
                                                        {teacher.phone && <div className="flex items-center gap-1.5 text-xs text-muted-foreground"><PhoneIcon className="w-3 h-3"/> {teacher.phone}</div>}
                                                    </div>
                                                </td>
                                                ...
                                                <td className="p-4 text-right" onClick={(e) => e.stopPropagation()}>
                                                    <button className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground transition-colors"><MoreHorizontalIcon className="w-5 h-5"/></button>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
