            ...
            <form onSubmit={handleSignup} className="space-y-6 md:space-y-8 relative z-10">
                ...
                <div className="space-y-3 group">
                    ...
                    <div className="relative">
                        ...
                        <input name="displayName" value={formData.displayName} onChange={handleChange} required className="block w-full h-[64px] md:h-[72px] pl-16 pr-6 bg-black/40 border border-white/5 rounded-2xl text-sm text-white placeholder:text-white/10 focus:outline-none focus:ring-8 focus:ring-primary/5 focus:border-primary focus:bg-black/60 transition-all duration-300 font-bold tracking-wider" placeholder="FULL LEGAL NAME" />
                    </div>
                </div>
                ...
                <div className="space-y-3 group">
                    ...
                    <div className="relative">
                        ...
                        <input name="email" type="email" value={formData.email} onChange={handleChange} required className="block w-full h-[64px] md:h-[72px] pl-16 pr-6 bg-black/40 border border-white/5 rounded-2xl text-sm text-white placeholder:text-white/10 focus:outline-none focus:ring-8 focus:ring-primary/5 focus:border-primary focus:bg-black/60 transition-all duration-300 font-mono font-bold tracking-wider" placeholder="ID@NODE.NET" />
                    </div>
                </div>
                ...
                <div className="space-y-3 group">
                    ...
                    <div className="relative">
                        ...
                        <input name="password" type={showPassword ? "text" : "password"} value={formData.password} onChange={handleChange} required className="block w-full h-[64px] md:h-[72px] pl-16 pr-16 bg-black/40 border border-white/5 rounded-2xl text-sm text-white placeholder:text-white/10 focus:outline-none focus:ring-8 focus:ring-primary/5 focus:border-primary focus:bg-black/60 transition-all duration-300 font-mono font-bold tracking-widest" placeholder="••••••••" />
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-6 flex items-center text-white/20 hover:text-white transition-colors">{showPassword ? <EyeOffIcon className="h-5 w-5" /> : <EyeIcon className="h-5 w-5" />}</button>
                    </div>
                </div>

                <div className="pt-4">
                    <button type="submit" disabled={loading} className="w-full h-[64px] md:h-[72px] flex items-center justify-center py-3.5 px-8 rounded-2xl shadow-2xl shadow-primary/20 text-[11px] font-black text-white bg-primary hover:bg-primary/90 transition-all transform hover:-translate-y-1 active:scale-95 disabled:opacity-50 uppercase tracking-[0.5em]">
